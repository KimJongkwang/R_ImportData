...
R은 다음과 같은 방법으로 `dat`라는 데이터 프레임에 데이터를 저장할 수 있다. (버전 4부터는 `stringsAsFactors`의 기본값이 `FALSE`로 변경되었다.)
```{r dataframe, tidy=FALSE}
datMsg <- 
  data.frame(
...
* 동일한 데이터를 R에서 읽어들일 수 있도록 문서편집기로 직접 텍스트 화일을 작성해보자.

### 확장자 `csv`{#readcsv}

테이블의 형식의 데이터는 다양한 형태로 화일로 저장될 수 있다. 구체적인 형태는 흔히 화일의 확장자로부터 유추할 수 있다. 예를 들어 csv는 **c**omma **s**eperated **v**alue(쉼표로 구분된 값), tsv는 **t**ab **s**eperated **v**alue(탭으로 구분된 값)의 약자이다. 하지만 csv 확장자의 텍스트 화일이 반드시 쉼표(comma)로 열이 구분되는 것은 아니다. R에서 csv 화일을 불러들이는 쉬운 방법은 `read.csv` 함수를 사용하는 것이다. 이때 주의할 점이 있다. 다음을 보자.

```{r readcsv, collapse=T, eval=F}
...
```{r readcsv02, collapse=T, tidy=FALSE}
datMsg03 <- 
